...
# 添加*单张*解模糊接口的路由
@app.route('/image_deblurring/solo_pic', methods=['POST', 'GET'])  # 添加路由
def image_deblurring_solo():
    if request.method == 'POST':
        start = time.time()
        solo_pic = request.files['solo_pic']
        ...
        # 修改上传的文件名
        unix_time = int(time.time())
        new_pname = str(unix_time) + '.png'
        # 上传文件保存路径
        upload_path = os.path.join(basepath, 'SRN/testing_set')
        upload_dir = os.path.join(basepath + '/SRN/testing_set/%s' % new_pname)
        # 清除历史文件
        if os.path.isdir(upload_path):
            pass
            # shutil.rmtree(upload_path)
        ...
        # 使用一个字典封装返回结果
        solo_rlt = {}
        solo_rlt['method'] = 'deblurring'

        run_model.main()
        local_pic = basepath + '/SRN/testing_res/%s' % new_pname
        print(local_pic)
        remote_pic = '/home/XYZ/images/%s' % new_pname
        upload_status = upload_pic(local_pic, remote_pic)
        # 将图片服务器对应的地址放入结果中返回
        if upload_status == 1:
            solo_rlt['solo_path'] = '/SRN/testing_res/%s' % new_pname
        elif upload_status == 0:
            solo_rlt['solo_path'] = 'None'
        end = time.time()
        print("处理耗时: %s" % (end - start))
...
# 添加去模糊的批处理图片的路由
@app.route('/image_deblurring/batch', methods=['POST', 'GET'])
def image_deblurring_batch():
    if request.method == 'POST':
        start = time.time()
        # 接收前端传过来的一个压缩文件
        try:
        ...
        base_path = os.path.dirname(__file__)
        print("当前运行的目录路径为： ", base_path)
        # 将接收到的压缩包*解压*到指定目录folder_path下(floder_path为 SRN/testing_set）

        folder_path = os.path.join(base_path + '/SRN/testing_set')
        un_zip(batch, folder_path)

        # 运行处理程序（从testing_set中批量处理，结果保存到testing_res中）
        run_model.main()
        res_path = os.path.join(base_path+'/SRN/testing_res')

        print('批量去模糊完毕，开始打包进行上传！')

        # 将结果文件夹*压缩*成zip包，命名为 "时间戳.zip"
        batch_name = int(time.time())
        zipped_file = go_zip(base_path + '/tmp/%d.zip' % batch_name, res_path)
...
if __name__ == '__main__':
    # 设置host为'0.0.0.0'，可以本地运行ip：127.0.0.1，加端口号访问。因为没有前端，无法用浏览器发送HTTP的Post请求，所以使用postman测试POST请求
